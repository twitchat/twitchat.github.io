...
subject.debounce(3000)
    ...
    .doOnNext(function (msg) {
        ...
        var votes = msg.replace(/.*\?/, "").split(':');
        $('#question').text(question);
        for (var i = 0, n = votes.length; i < n; i++) {
            //<div><span>0</span><a href="">Vote</a>PHP</div>
            $('#vote').append("<div id=\"vote" + (i+1) +"\"><span>0</span><a href=\"\">!vote " + (i+1) + "</a>" + votes[i] + "</div>");
        }
        $("#vote").show();
    })
    .debounce(5000)
...
Rx.Observable.just(0)
    .delay(15000).subscribe(function (i) {
        $("#vote").hide();
    })

function handleChat(channel, user, message, self) {
    if (message.trim().startsWith('!votes ')) {
        subject.onNext(message.trim().slice('!votes '.length).trim());
    } else if (message.trim().startsWith('!vote ')) {
        var n = message.trim().slice('!vote '.length).trim();
        console.log(n);
        var i = parseInt(n);
        console.log(i);
        if (i) {
